...
def run_game_scenario_1(base_url: str, scenario: int, player_id: str, seed: int = 42, samples: int = 200_000):
    """
    Runner specifically for Scenario 1.
    Scenario 1 format: constraints as array of {attribute, minCount} objects
    """
    print("Creating game client...")
    gc = GameClient(base_url)
    print("Calling new_game API...")
    meta = gc.new_game(scenario, player_id)
    print("Got game metadata for Scenario 1")

    game_id = meta["gameId"]
    constraints = meta["constraints"]  # Array format: [{"attribute": "young", "minCount": 600}, ...]
