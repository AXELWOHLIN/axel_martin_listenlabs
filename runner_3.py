...
def run_game_scenario_3(base_url: str, scenario: int, player_id: str, seed: int = 42, samples: int = 200_000):
    """
    Runner specifically for Scenario 3.
    Scenario 3 format: constraints as array of {attribute, minCount} objects (same as Scenario 1)
    """
    print("Creating game client...")
    gc = GameClient(base_url)
    print("Calling new_game API...")
    meta = gc.new_game(scenario, player_id)
    print("Got game metadata for Scenario 3")

    game_id = meta["gameId"]
    constraints = meta["constraints"]  # Array format: [{"attribute": "underground_veteran", "minCount": 500}, ...]
    stats = meta["attributeStatistics"]

    # Extract constrained attributes and targets from array format
